...
-- =====================================================
-- NOTE: this dump stops tracking at migration 014 and resumes at 022.
-- Migrations 015-021 live only in supabase/migrations/ — a fresh install built
-- from this file alone is missing them. Apply the numbered migrations instead.
-- =====================================================

-- 022 — Server-side recurring transaction automation + Pro gate
-- Recurring rules never fired before this: the browser-side processor was never
-- called, so execution moves into the database where it runs whether or not the
-- app is open.
...
-- VIP check honouring both sources of truth (users.is_vip/vip_until AND subscriptions).
...
-- Recurring is Pro-only. Read and delete stay open so a lapsed member can still
-- see and clean up rules they created while paying.
...
-- Next occurrence after p_from, or the first one after p_min when given. The
-- monthly day is clamped to 28 so a rule made on the 31st can't roll over.
...
-- Wallet balance is maintained by the migration 017 insert trigger, so posting
-- here keeps balances correct without duplicating that arithmetic.
